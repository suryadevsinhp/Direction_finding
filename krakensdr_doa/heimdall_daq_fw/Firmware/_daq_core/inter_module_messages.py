...
def pack_msg_lna_ctr(module_identifier, state):
    """
        Prepares the byte array of an inter-module ZMQ message for external LNA control
        
        Parameters:
        -----------
            :param: module_identifier: Source module id
            :param: state: True will turn on the lna, False will turn off the lna
    ...
    msg_length = 128 # Total message length 128 byte
    msg_byte_array  = pack("b", module_identifier) # 1byte
    msg_byte_array += 'l'.encode('ascii') # 1 byte
    if state:
