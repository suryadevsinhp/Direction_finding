...
#define ASYNC_BUF_NUMBER 64 // Number of buffers used by the asynchronous read 
 ...
 */    
{
    ...
    while(!exit_flag){
        ...
        else if (msg->command_identifier == 'l')
        {
            log_info("------------------ LNA request ---------------------");            
            uint32_t * parameters = (uint32_t * ) msg->parameters;            
            lna_state = parameters[0];
            lna_change_flag = 1;
...
{   
    ...
    /* Animation for antenna ---------------------------------------------------*/
     // all off
     for(int m=0;m<ch_no;m++)
    {   
        if(m>4)
                {
                    struct rtl_rec_struct* rtl_rec_aux=&rtl_receivers[5];
                    rtlsdr_set_bias_tee_gpio(rtl_rec_aux->dev, m-4, 0);
                }
        else
            {
                struct rtl_rec_struct *rtl_rec = &rtl_receivers[ctr_channel_index]; 
                rtlsdr_set_bias_tee_gpio(rtl_rec->dev, m+1, 0);
            }
    }
    // initialize
     for(int m=0;m<ch_no;m++)
    {   
        if(m>4)
                {
                    struct rtl_rec_struct* rtl_rec_aux=&rtl_receivers[5];
                    rtlsdr_set_bias_tee_gpio(rtl_rec_aux->dev, m-4, 1);
                }
        else
            {
                struct rtl_rec_struct *rtl_rec = &rtl_receivers[ctr_channel_index]; 
                rtlsdr_set_bias_tee_gpio(rtl_rec->dev, m+1, 1);
            }
            usleep(200000);
    }
    for(int m=ch_no-1; m>-1; m--)
    {   
        log_info(" %d disabled",m);
        if(m>4)
                {
                    struct rtl_rec_struct* rtl_rec_aux=&rtl_receivers[5];
                    rtlsdr_set_bias_tee_gpio(rtl_rec_aux->dev, m-4, 0);
    ...
    {   
        ...
        if(m>4)
                {
                    log_info("Enabling Bias T on Receiver 2");
                    struct rtl_rec_struct* rtl_rec_aux=&rtl_receivers[5];
                    rtlsdr_set_bias_tee_gpio(rtl_rec_aux->dev, m-4, en_bias_tee[m]);
                }
        else
            {
                ...
                */
                ...
                if(ch_no>4)
                {
                    ...
                    if(noise_source_state == 1)
                        ...
                        rtlsdr_set_bias_tee_gpio(rtl_rec_aux->dev, 0, 1);
                    else if(noise_source_state == 0)
                        //rtlsdr_set_gpio(rtl_rec_aux->dev, 0, 0);
                        rtlsdr_set_bias_tee_gpio(rtl_rec_aux->dev, 0, 0);
                }
