...
class HWC():
    ...
    def _handle_control_reqest(self, command, params):
        ...
        if command == "FREQ":            
            ...
            self._control_lna(int(params[0]))
    ...
    def _control_lna(self, lna_state):
        if lna_state:
           
            self.logger.info("Enable LNAs")
            msg_byte_array = inter_module_messages.pack_msg_lna_ctr(self.module_identifier, True)
            self.rtl_daq_socket.send(msg_byte_array)
            reply = self.rtl_daq_socket.recv()
            self.logger.debug(f"Received reply: {reply}")                 
            self.last_lna = True 
            ...
            self.logger.info("Enable LNAs")
            msg_byte_array = inter_module_messages.pack_msg_lna_ctr(self.module_identifier, False)
            self.rtl_daq_socket.send(msg_byte_array)
            reply = self.rtl_daq_socket.recv()
            self.logger.debug(f"Received reply: {reply}")                 
            self.last_lna = False 
...
class CtrIfaceServer(threading.Thread):
    ...
    def process_ctr_frame(self, msg_bytes):
        ...
        if command == "EXIT":
            ...
            lna_state = unpack('Q',msg_bytes[4:12])[0]
            self.logger.info("Received lna Request: {:f} ".format(lna_state))
            ctr_request.clear()
            ctr_request.append(command)
            ctr_request.append(lna_state)
