...
def update_dsp_params(
    ...
    if max_phase_diff > 0.5:
        ambiguity_warning = "WARNING: Array size is too large for this frequency. DoA estimation is ambiguous. Max phase difference:{:.1f}Â°.".format(
            np.rad2deg(2 * np.pi * max_phase_diff)
        )
    elif max_phase_diff < 0.1:
        ambiguity_warning = "WARNING: Array size may be too small.  Max phase difference: {:.1f}Â°.".format(
            np.rad2deg(2 * np.pi * max_phase_diff)
        )
    else:
        ambiguity_warning = ""
...
@app.server.route('/update_df_params/<frq>/<gain>/<spacing>')
def callback(frq, gain, spacing):
    try:

        # parser
        gain = gain.replace("_", ".")
        frq = frq.replace("_", ".")
        spacing = spacing.replace("_", ".")

        # test_text = int(code)
        f0 = float(frq)
        gain = float(gain)

        web_interface.ant_spacing_meters = float(spacing)
        ...
        if web_interface.module_signal_processor.DOA_ant_alignment == "UCA":
            ...
            inter_elem_spacing = (np.sqrt(2) * web_interface.ant_spacing_meters * np.sqrt(
        ...
        for i in range(web_interface.module_signal_processor.max_vfos):
            web_interface.module_signal_processor.vfo_freq[i] = int(float(frq) * 1e6)

        web_interface.save_configuration()

        to_return = "Frequency : " + str(frq) + " MHz | Gain : " + str(gain) + " dBm | Spacing (m) :" + str(spacing)
        return to_return
    except Exception as error:
        return str(error)


@app.server.route('/update_doa_algorithm/<doa_algorithm>')
def callback_doa_algorithm(doa_algorithm):
    try:
        
        web_interface.module_signal_processor.DOA_algorithm = doa_algorithm

        web_interface.save_configuration()

        to_return = web_interface.module_signal_processor.DOA_algorithm
        return to_return
    except Exception as error:
        return str(error)

@app.server.route('/set_freq/<frq>')
def callback_freq(frq):
    try:
        ...
        to_return = "Frequency : " + str(frq) 
        return to_return
    except Exception as error:
        return str(error)


@app.server.route('/set_gain/<gain>')
def callback_gain(gain):
    try:
        # parser
        gain = gain.replace("_", ".")
        gain = float(gain)
        web_interface.config_daq_rf(web_interface.daq_center_freq, gain)

        web_interface.save_configuration()

        to_return = "Gain : " + str(gain) 
        return to_return
    except Exception as error:
        return str(error)


@app.server.route('/set_array_radius/<spacing>')
def callback_spacing(spacing):
    try:
        ...
        web_interface.ant_spacing_meters = float(spacing)
        web_interface.save_configuration()
        to_return = "Radius (m) :" + str(spacing)
        return to_return
    except Exception as error:
        return str(error)

@app.server.route('/set_compass_offset/<offset>')
def callback_compass_offset(offset):
    
    offset =  int(offset)

    web_interface.compass_offset = offset
    web_interface.module_signal_processor.compass_offset = offset
    web_interface.save_configuration()
    
    return "Compass Offset : " + str(offset)


@app.server.route('/restart_subsystem')
def callback_restart_subsystem():
...
@app.server.route('/reboot_subsystem')
def callback_reboot_subsystem():
...
@app.server.route('/set_array_offset/<offset>')
def callback_array_offset(offset):
    
    offset =  int(offset)
    web_interface.module_signal_processor.array_offset = offset
    web_interface.save_configuration()
    
    return "Array Offset : " + str(offset)




@app.server.route('/get_angle')
def get_angle():
    try:
        
        return json.dumps({"aoa": str(((360 - web_interface.doas[0] + web_interface.compass_offset) % 360)),
            "max_amplitude" : "{:.1f}".format(web_interface.max_amplitude),
            "polar": web_interface.doa_results[0].tolist(),
            "thetas" : ((360 - web_interface.doa_thetas + web_interface.compass_offset) % 360).tolist()}) 
    except Exception as error:
        return str(error)
    # return str(angle) + "," + str(max_power)+ ',' + str(web_interface.doa_results)


@app.server.route('/get_phases')
def get_phases():
    try:
        with open('/home/krakenrf/phase_details.json', 'r') as openfile:
...
@app.server.route('/control_antenna/<ant>')
def control_antenna(ant):
    try:
        states = {"antenna" : str(ant),"noise" : 1}
        
        with open("/home/krakenrf/gpio_states.json", "w") as outfile:
...
@app.server.route('/control_lna/<state>')
def control_lna(state):
    try:
        web_interface.set_lna(int(state))
        return str(state)
    except:
        return "Failed"
    
    
@app.server.route('/get_gps')
def get_gps():
    try:
        with open('/home/krakenrf/gps.json', 'r') as openfile:
            json_object = json.load(openfile)
            print(json_object)
...
@app.server.route('/coldstart_gps')
def coldstart_gps():
    try:
        with open('/home/krakenrf/gps.json', 'r') as openfile:
            json_object = json.load(openfile)
        ...
        json_object["cold_start"] = 1
        
        with open('/home/krakenrf/gps.json', 'w') as file:
...
@app.server.route('/get_heading')
def getheading():
    try:
        with open('/home/krakenrf/compass.json', 'r') as openfile:
            json_object = json.load(openfile)
            print(json_object)
...
@app.server.route('/calibrate_compass')
def calirate_comp():
    try:
        with open('/home/krakenrf/compass.json', 'r') as openfile:
            json_object = json.load(openfile)
        ...
        json_object["calibrate"] = 1
        
        with open('/home/krakenrf/compass.json', 'w') as file:
            json.dump(json_object, file, indent=4)
...
@app.server.route('/init_subsystem')
def init_subsystem():
    try:
        fetch_dsp_data(app, web_interface, spectrum_fig, waterfall_fig)
        fetch_gps_data(app, web_interface)
...
@app.server.route('/update_bw_sq/<bw>/<sq>')
def update_bw_sq(bw, sq):
    bw = float(bw.replace("_", "."))
    sq = int(sq.replace("_", "."))

    for i in range(web_interface.module_signal_processor.max_vfos):
        web_interface.module_signal_processor.vfo_bw[i] = int(bw * 1e6)
        web_interface.module_signal_processor.vfo_squelch[i] = int(sq)

    return "set"

@app.server.route('/set_vfo_freq/<frq>')
def set_vfo_freq(frq):
    frq = float(frq.replace("_", "."))
    for i in range(web_interface.module_signal_processor.max_vfos):
        web_interface.module_signal_processor.vfo_freq[i] = int(frq * 1e6)

    return "vfo freq set to " + str(frq)

@app.server.route('/check_alive')
def check_alive():
    return "ok"


@app.server.route('/get_status')
def get_status():
    try:
        status = {"frame_type": web_interface.daq_frame_type,
                  "connection_status": web_interface.daq_conn_status,
                  "frame_sync": web_interface.daq_frame_sync,
                  "iq_sync": web_interface.daq_iq_sync,
                  "sample_delay_sync": web_interface.daq_sample_delay_sync}
...
@app.server.route('/spectrum_antenna/<ant>')
def spect_antenna(ant):
    try:
        web_interface.module_signal_processor.spectrum_antenna = int(ant)
        return ant
